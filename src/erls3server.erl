...
-record(state, {ssl, access_key, secret_key, region, pending, timeout=?TIMEOUT}).
...
%% @end
%%--------------------------------------------------------------------
start_link([Access, Secret, Region, SSL, Timeout]) ->
    gen_server:start_link(?MODULE, [Access, Secret, Region, SSL, Timeout], []).
...
init([Access, Secret, Region, SSL, nil]) ->
    {ok, #state{ssl = SSL,access_key=Access, secret_key=Secret, region = Region, pending=gb_trees:empty()}};
init([Access, Secret, Region, SSL, Timeout]) ->
    {ok, #state{ssl = SSL,access_key=Access, secret_key=Secret, region = Region, timeout=Timeout, pending=gb_trees:empty()}}.
...
handle_call({link_to, Bucket, Key, Expires}, _From, #state{access_key=Access, secret_key=Secret, region=Region, ssl=SSL}=State)->
    Exp = integer_to_list(erls3util:unix_time(Expires)),
    QueryParams = [{"AWSAccessKeyId", Access},{"Expires", Exp}],
    Url = buildUrl(Region,Bucket,Key,QueryParams, SSL),
...
handle_info({ibrowse_async_response_end,RequestId}, State = #state{pending=P})->
    case gb_trees:lookup(RequestId,P) of
		{value,#request{started=Started, to_file=Fd, opts=Opts}=R} ->
		    ...
		    erls3:notify([{time, timer:now_diff(erlang:timestamp(), Started)/1000 }| Opts]),
			  {noreply,State#state{pending=gb_trees:delete(RequestId, P)}};
		none -> {noreply,State}
			%% the requestid isn't here, probably the request was deleted after a timeout
	end;
...
genericRequest(From, #state{ssl=SSL, access_key=AKI, secret_key=SAK, region=Region, timeout=Timeout, pending=P }=State,
    ...
    Url = buildUrl(Region,Bucket,Path,QueryParams, SSL),
        ...
        Fd ->
            case ibrowse:send_req(lists:flatten(Url), Headers,  Method, Contents,Options, Timeout) of
                {ibrowse_req_id,RequestId} ->
                    Pendings = gb_trees:insert(RequestId,#request{pid=From, to_file=Fd, opts=Params, started=erlang:timestamp(),  callback=Callback},P),
                ...
                {error, retry_later} ->
                    ?DEBUG("******* ERROR : retry_later \n",[]),
                    {reply, retry, State};
                {error, conn_failed} ->
                    ?DEBUG("******* ERROR : conn_failed \n",[]),
                    {reply, retry, State};
                {error, {conn_failed, Why}} ->
                    ?DEBUG("******* ERROR : conn_failed ~w\n", [Why]),
                    {reply, retry, State};
                {error, {send_failed, _}} ->
                    ?DEBUG("******* ERROR : send_failed \n",[]),
                    {reply, retry, State};
                {error, req_timedout} ->
                    ?DEBUG("******* ERROR :  req_timedout \n",[]),
                    {reply, retry, State};
                {error, E} ->
